...
'''
MODIFICATIONS:
- Added functionality to keep mongoDB and IB in sync.

TODO:
- Improve the stop-loss and take-profit logic to be more dynamic.
'''
import heapq
import logging
import time
...
from ib_insync import IB, MarketOrder, Stock, AccountValue, Trade, Fill
...
def register_exec_handler(ib: IB, mongo_client: MongoClient):
    """Call this once at startup to wire up the execDetailsEvent handler."""
    db         = mongo_client.trades
    fills_coll = db.paper
    assets     = db.assets_quantities
    limits     = db.assets_limit

    def on_exec(trade: Trade, fill: Fill):
        ...
        # 2) Update positions & SL/TP limits
        _update_positions(symbol, side, qty, assets, limits, price)

    ib.execDetailsEvent += on_exec

def _update_positions(symbol, side, qty, assets, limits, price):
    stop_loss, take_profit = 0.03, 0.05
    # (You could opt to compute SL/TP dynamically in process_ticker later)
...
def place_order(ib: IB, symbol: str, side: str, quantity: float):
...
def process_ticker(ticker, mongo_client, indicator_periods, strategy_to_coefficient, ib):
    ...
    if decision == "sell" and portfolio_qty > 0:
        sold = True
        place_order(ib, ticker, "SELL", max(quantity, 1))
    elif (decision == "buy"
...
def process_market_open(mongo_client, ib):
    ...
    execute_buy_orders(ib)


def execute_buy_orders(ib):
    global buy_heap, suggestion_heap, sold
...
def main():
    ...
    # 3) Execution details
    register_exec_handler(ib, mongo_client)
